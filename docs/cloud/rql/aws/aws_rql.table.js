...
export const queries = [
	...
	},
	{
		...
		rql:
			"config from cloud.resource where api.name = 'aws-cloudformation-describe-stacks' AND json.rule = stackResources[*].driftInformation.stackResourceDriftStatus equals DRIFTED",
	},
	{
	...
	},
	{
		...
		rql:
			'config from cloud.resource where resource.status = Active AND api.name = \'aws-lambda-list-functions\' AND json.rule = vpcConfig does not exist and handler does not equal "twistlock.handler" and environment.variables does not contain "TW_POLICY"',
	},
	{
...
export const columns = [
	...
	},
	{
		...
		Cell: ({ cell: { value }, row: { original } }) => <CodeBlock className="sql">{value}</CodeBlock>,
	},
];
